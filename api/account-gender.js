...
async function updateGender(token,institutionId,gender){
  ...
  const actorResponse=await fetch(`${url}/rest/v1/pire_profiles?id=eq.${encodeURIComponent(user.id)}&select=role,status&limit=1`,{headers:authHeaders});
  const [actor]=actorResponse.ok?await actorResponse.json():[];
  if(actor?.role!=="Yönetici"||actor?.status!=="Aktif")throw Object.assign(new Error("Yalnızca aktif yönetici işlem yapabilir."),{status:403});
...
handler._test={updateGender,originAllowed};module.exports=handler;
