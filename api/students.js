...
async function readAll(token){
  const [studentsRaw,paymentsRaw,packagesRaw,lessonsRaw,linksRaw]=await Promise.all([rest(token,"pire_students?select=*&order=full_name.asc"),rest(token,"pire_payments?select=*&order=billing_month.desc,id.desc"),rest(token,"pire_packages?select=id,student_id,course_name,status,created_at&order=created_at.desc,id.desc"),rest(token,"pire_lessons?select=id,course_name,teacher_name,lesson_date,start_time&order=lesson_date.desc,start_time.desc,id.desc"),rest(token,"pire_lesson_students?select=lesson_id,student_id")]);
  const students=(studentsRaw||[]).map(studentRow),byId=new Map(students.map(s=>[String(s.id),s]));const activeSeen=new Set();for(const p of packagesRaw||[]){const key=String(p.student_id);if(p.status==="Aktif"&&!activeSeen.has(key)&&byId.has(key)){byId.get(key).course=p.course_name||"";activeSeen.add(key)}}
...
module.exports=async function handler(req,res){
  try{
    ...
    if(req.method==="GET")return send(res,200,await readAll(token));
...
module.exports._test={studentRow,paymentRow,normalize};
