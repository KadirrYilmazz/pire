...
(async()=>{
  ...
  await run("öğrenci ödeme özeti yalnızca RLS ile okunan sunucu verisinden gelir",async()=>{let sent;const now=new Date(),billingMonth=`${now.getUTCFullYear()}-${String(now.getUTCMonth()+1).padStart(2,"0")}-01`;process.env.GROQ_API_KEY="server-only-groq-test-key";mockGroq({role:"Öğrenci",capture:value=>{sent=JSON.stringify(value)},content:JSON.stringify({answer:"Bu ay 750 TL ödemeniz kaldı.",intent:"self.payment.view"}),tables:{pire_ai_students:[{id:9203,status:"Aktif",monthly_fee:1500,payment_day:12}],pire_ai_payments:[{student_id:9203,billing_month:billingMonth,amount_due:1500,amount_paid:750,status:"Kısmi"}]}});const res=response();await handler(request({token:"valid",body:{question:"Bu ay ne kadar ödeme yapmalıyım?",summary:{metric:"institution_overview",finance:{totalBalance:999999},studentName:"GİZLİ"}}}),res);delete process.env.GROQ_API_KEY;assert.equal(res.statusCode,200);assert.equal(res.body.task.intent,"self.payment.view");assert.match(sent,/\\"balance\\":750/);assert.doesNotMatch(sent,/999999|GİZLİ|9203/)});
