(()=>{
'use strict';if(window.__PIRE_VISITOR_NAV_LAYOUT_V14__)return;window.__PIRE_VISITOR_NAV_LAYOUT_V14__=true;
const labels=['Ana Sayfa','Akademik Koçluk','Enstrüman Eğitimi','Prodüktörlük ve Stüdyo Kaydı','Organizasyon','İletişim'];
const norm=s=>String(s||'').replace(/\s+/g,' ').trim();
const visible=el=>{if(!el)return false;const r=el.getBoundingClientRect(),st=getComputedStyle(el);return r.width>0&&r.height>0&&st.display!=='none'&&st.visibility!=='hidden'};
...
const has=txt=>[...document.querySelectorAll('button,a,[role="button"]')].some(el=>visible(el)&&norm(el.textContent).includes(txt));
const admin=()=>has('Çıkış Yap')||has('Finans')||has('Öğrenciler')||has('Eğitmenler')||has('Raporlar')||has('Kullanıcılar');
let visitorSince=0;function visitorReady(){if(admin()){visitorSince=0;return false}if(!find('Giriş Yap')){visitorSince=0;return false}if(!visitorSince)visitorSince=Date.now();return Date.now()-visitorSince>=400}
...
function cleanup(){document.querySelectorAll('[data-pire-vnav-nav="1"]').forEach(nav=>{navProps.forEach(p=>nav.style.removeProperty(p));nav.removeAttribute('data-pire-vnav-nav')});document.querySelectorAll('[data-pire-vnav-item="1"]').forEach(el=>{itemProps.forEach(p=>el.style.removeProperty(p));el.removeAttribute('data-pire-vnav-item')})}
function apply(){
 if(!visitorReady()){cleanup();return}
 const items=labels.map(find);if(items.some(x=>!x))return;const nav=items[0].parentElement;if(!nav)return;
 ...
 nav.setAttribute('data-pire-vnav-nav','1');
