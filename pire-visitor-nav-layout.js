(()=>{
'use strict';if(window.__PIRE_VISITOR_NAV_LAYOUT_V12__)return;window.__PIRE_VISITOR_NAV_LAYOUT_V12__=true;
...
function accessToken(){try{for(let i=0;i<localStorage.length;i+=1){const key=localStorage.key(i)||'';if(!/^sb-.*-auth-token$/.test(key))continue;const v=JSON.parse(localStorage.getItem(key)||'null');const token=v?.access_token||v?.currentSession?.access_token;if(token)return token}}catch(_){}return ''}
const visitor=()=>!accessToken()&&!!find('Giriş Yap');
...
function cleanup(){
 document.querySelectorAll('[data-pire-vnav-nav="1"]').forEach(nav=>{navProps.forEach(p=>nav.style.removeProperty(p));nav.removeAttribute('data-pire-vnav-nav')});
 document.querySelectorAll('[data-pire-vnav-item="1"]').forEach(el=>{itemProps.forEach(p=>el.style.removeProperty(p));el.removeAttribute('data-pire-vnav-item')});
}
function apply(){
...
function start(){apply();mo.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['class','aria-current']});setTimeout(apply,300);setTimeout(apply,1000)}
if(document.body)start();else addEventListener('DOMContentLoaded',start,{once:true});
addEventListener('resize',rerun);addEventListener('storage',rerun);
})();