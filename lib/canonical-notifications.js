...
async function getState(token,profileId){
  const [prefRows,readRows,students,payments,paymentTransactions,packages,lessons,links,attendance,progress,makeups,announcements,announcementReads]=await Promise.all([
    ...
    rest(token,'pire_student_directory?select=student_id,full_name'),
    rest(token,'pire_payments?select=id,student_id,billing_month,amount_due,status'),
    rest(token,'pire_payment_transactions?select=payment_id,amount'),
  ...
  for(const tx of paymentTransactions||[]){const key=String(tx.payment_id||'');if(!key)continue;paidByPayment.set(key,(paidByPayment.get(key)||0)+asNumber(tx.amount))}
