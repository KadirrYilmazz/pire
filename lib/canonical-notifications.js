...
async function getState(token,profileId){
  const [prefRows,readRows,students,payments,packages,lessons,links,attendance,progress,makeups,announcements,announcementReads]=await Promise.all([
    ...
    rest(token,'pire_students?select=id,full_name,payment_day'),rest(token,'pire_payments?select=id,student_id,billing_month,amount_due,amount_paid,status'),
  ...
  const prefs={...DEFAULTS,...(prefRows?.[0]?.preferences||{})},reads=new Set((readRows||[]).map(x=>String(x.notification_key))),annRead=new Set((announcementReads||[]).map(x=>String(x.announcement_id))),studentNames=new Map((students||[]).map(x=>[String(x.id),x.full_name]));
